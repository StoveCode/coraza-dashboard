<template>
  <div class="bg-gray-900 border border-gray-800 rounded-xl p-5">
    ...
    <div v-show="infoOpen" class="mb-4 bg-gray-800/60 border border-gray-700 rounded-lg p-3 text-xs text-gray-400 space-y-1">
      <p>Inbound rules detect attacks in requests — SQL Injection, XSS, RCE, LFI etc. Score accumulates per transaction, block triggers when threshold is reached.</p>
      <div class="pt-1 space-y-0.5">
        <p>• <span class="text-gray-300">Typical rule scores:</span> Critical=5, Error=4, Warning=3, Notice=2</p>
        <p>• <span class="text-gray-300">Lower threshold = more aggressive</span> (fewer points needed to block)</p>
      </div>
    </div>
